  ...
  methods: {
    ...
    showHandCards (tails) {
      ...
      const newTailsM = tails.m.map(num => {
        if (num == '0') { return '5' } else { return num }
      })
    ...
    transArr (CardString) { // 将m=['0','5']转变为[1,0,0,0,0,1,0,0,0]
      const arr_m = new Array(9).fill(0)
      const arr_p = new Array(9).fill(0)
      const arr_s = new Array(9).fill(0)
      const arr_z = new Array(7).fill(0)
      ...
      const arr = [[...arr_m], [...arr_p], [...arr_s], [...arr_z]]
      return arr
    },
    sum (arr) { // 当前花型有几张牌
      let s = 0
        ...
        { // 先算面子，再算顺子
          ...
          for (let i = 0; i < 9; i++) {
            ...
            if (!is_jihai) { // 非字牌有顺子，考虑有2个顺子或者4个顺子
              if (arr2[i] >= 2 && arr2[i + 1] >= 2 && arr2[i + 2] >= 2) { arr2[i] -= 2, arr2[i + 1] -= 2, arr2[i + 2] -= 2, tmp_mentsu += 2 }
              if (arr2[i] >= 2 && arr2[i + 1] >= 2 && arr2[i + 2] >= 2) { arr2[i] -= 2, arr2[i + 1] -= 2, arr2[i + 2] -= 2, tmp_mentsu += 2 }
            }
            ...
            if (arr2[i] > 0 && arr2[i + 1] > 0) { arr2[i]--, arr2[i + 1]--, tmp_tatsu++ }
            if (arr2[i] > 0 && arr2[i + 2] > 0) { arr2[i]--, arr2[i + 2]--, tmp_tatsu++ }
          }
